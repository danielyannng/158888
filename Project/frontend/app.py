...
def seed_feedback(count=420, use_real=False):
    if use_real:
        response = requests.post(_api_url("/api/seed/real?reset=true"), timeout=20)
    else:
        response = requests.post(_api_url("/api/seed"), json={"count": count, "reset": True, "use_real": use_real}, timeout=20)
    response.raise_for_status()
    st.cache_data.clear()
    return response.json()
...
with st.sidebar:
    ...
    refresh_col, seed_col, real_col = st.columns(3)
    with refresh_col:
        if st.button("Refresh Data", use_container_width=True):
            st.cache_data.clear()
            st.rerun()
    with seed_col:
        if st.button("Seed Demo", use_container_width=True, disabled=not backend_ok):
            try:
                seed_feedback()
                st.success("Seeded synthetic data")
                st.rerun()
            except requests.RequestException as exc:
                st.error(f"Seed failed: {exc}")
    with real_col:
        if st.button("Real Data", use_container_width=True, disabled=not backend_ok, help="Load real Shanghai Metro passenger feedback"):
            try:
                seed_feedback(use_real=True)
                st.success("Seeded real feedback data")
                st.rerun()
            except requests.RequestException as exc:
                st.error(f"Seed failed: {exc}")
    st.markdown("---")
