...
def district_compare() -> list[dict]:
    ...
    Returns one dict per district with counts, neg_rate, and chi-square p-value
    ...
    # Build contingency table: rows = districts, cols = [negative, non-negative]
    rows_list = []
    district_names: list[str] = []
    for name, counts in district_counts.items():
        total = counts["positive"] + counts["neutral"] + counts["negative"]
        non_neg = counts["positive"] + counts["neutral"]
        if total > 0:
            rows_list.append([counts["negative"], non_neg])
    ...
    if len(rows_list) >= 2:
        try:
            chi2_stat, p_val, _, _ = chi2_contingency(rows_list)
        except Exception:  # noqa: BLE001
            chi2_stat, p_val = None, None
    ...
    for i, name in enumerate(district_names):
        ...
        # Per-category counts for this district
        for s in stations:
            if _assign_district(s["lat"], s["lon"]) == name:
                pass  # station-level, not category-level here
